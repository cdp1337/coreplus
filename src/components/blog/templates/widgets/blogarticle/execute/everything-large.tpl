...
<section class="blog-article-widget blog-article-widget-{$sort} blog-article-widget-everything-large">
	{if $title}
		<h3>{$title}</h3>
	{/if}
	...
	{foreach $links as $l}
		<article class="blog-article-widget-item" itemscope="itemscope" itemtype="http://schema.org/BlogPosting">
			...
			{if $l->getImage()}
				{a href="`$l.baseurl`"}
					{img class="blog-article-widget-img" placeholder="blog" file=$l->getImage() width='400' height='400' itemprop="thumbnailUrl" alt="`$l.title|escape`" includemeta=0}
				{/a}
			{/if}
