...
class WhoisClient {
	 ...
	 * @var array
	 */
	public static $WHOIS_PARAM = array(
	 ...
	 * @var array
	 */
	public static $WHOIS_SPECIAL = array(
		'ad'	 => '',
	 ...
	 * @var array
	 */
	public static $WHOIS_GTLD_HANDLER = array(
	 ...
	 * Non ICANN TLD's
	 */
	public static $WHOIS_NON_ICANN = array (
	...
	*/
	...
	function Process (&$result, $deep_whois=true) {
		...
		$object = 'phpwhois\\' . $handler_name.'_handler';
	...
	function DeepWhois ($query, $result) {
		...
		if (!empty($subresult))
			...
			if (isset(WhoisClient::$WHOIS_GTLD_HANDLER[$wserver]))
				$this->Query['handler'] = WhoisClient::$WHOIS_GTLD_HANDLER[$wserver];
			else
				{
