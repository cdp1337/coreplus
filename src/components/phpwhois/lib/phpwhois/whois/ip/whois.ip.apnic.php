...
class apnic_handler {
	function parse($data_str, $query) {
		...
		if(empty($blocks) || !is_array($blocks['main'])) {
			...
			while(list($key, $val) = each($contacts)) if(isset($rb[ $key ])) {
				if(is_array($rb[ $key ])) $blk = $rb[ $key ][ count($rb[ $key ]) - 1 ];
				else
					$blk = $rb[ $key ];
			...
			if(isset($r['network']['address'])) {
				if(isset($r['owner']['address'])) $r['owner']['address'][] = $r['network']['address'];
				else
