...
function generic_parser_a($rawdata, $translate, $contacts, $main = 'domain', $dateformat = 'dmy') {
	...
	if(isset($disclaimer) && is_array($disclaimer)) $ret['disclaimer'] = $disclaimer;
	...
	while(list($key, $val) = each($contacts)) if(isset($r[ $key ])) {
		if(is_array($r[ $key ])) $blk = $r[ $key ][ count($r[ $key ]) - 1 ];
		else
			$blk = $r[ $key ];
...
function generic_parser_a_blocks($rawdata, $translate, &$disclaimer) {
	...
	while(list($key, $val) = each($rawdata)) {
		...
		if(isset($block[ $k ]) && is_array($block[ $k ])) $block[ $k ][] = $v;
		else if(!isset($block[ $k ]) || $block[ $k ] == '') $block[ $k ] = $v;
		else {
			$x = $block[ $k ];
			unset($block[ $k ]);
			$block[ $k ][] = $x;
			...
			while(list($match, $field) = each($items)) {
				...
				if($pos !== false) {
					if($field != '') {
						...
						if($itm != '') eval($var . '="' . str_replace('"', '\"', $itm) . '";');
					}
...
function easy_parser($data_str, $items, $date_format, $translate = false, $has_org = false, $partial_match = false,
	$def_block = false) {
...
function get_contacts($array, $extra_items = '', $has_org = false) {
	if(isset($array['billing'])) $array['billing'] = get_contact($array['billing'], $extra_items, $has_org);

	if(isset($array['tech'])) $array['tech'] = get_contact($array['tech'], $extra_items, $has_org);

	if(isset($array['zone'])) $array['zone'] = get_contact($array['zone'], $extra_items, $has_org);

	if(isset($array['admin'])) $array['admin'] = get_contact($array['admin'], $extra_items, $has_org);

	if(isset($array['owner'])) $array['owner'] = get_contact($array['owner'], $extra_items, $has_org);

	if(isset($array['registrar'])) $array['registrar'] = get_contact($array['registrar'], $extra_items, $has_org);
...
function get_contact($array, $extra_items = '', $has_org = false) {
	...
	if($extra_items) {
		foreach($items as $match => $field) {
			if(!isset($extra_items[ $match ])) $extra_items[ $match ] = $field;
		}
		$items = $extra_items;
	}
	...
	if(!empty($array)) {
		if(isset($r['address'])) $r['address'] = array_merge($r['address'], $array);
		else
			$r['address'] = $array;
	}
...
function get_date($date, $format) {
	...
	if($res['y'] < 70) $res['y'] += 2000;
	else if($res['y'] <= 99) $res['y'] += 1900;
