<?php
/**
 * whois registration file for nz. TLD
 *
 * @package phpwhois
 *
...
if(!defined('__NZ_HANDLER__')) define('__NZ_HANDLER__', 1);
...
class nz_handler {
	function parse($data_str, $query) {
		...
		if(!empty($r['regrinfo']['domain']['status'])) $domain_status =
			substr($r['regrinfo']['domain']['status'], 0, 3);
		else
			$domain_status = '';

		if($domain_status == '200') $r['regrinfo']['registered'] = 'yes';
		elseif($domain_status == '220') $r['regrinfo']['registered'] = 'no';
		else
