...
if(isSet($_GET['query'])) {
	...
	if(!empty($_GET['output'])) $output = $_GET['output'];
	else
		$output = '';
	...
	switch($output) {
		...
		case 'nice':
			if(!empty($result['rawdata'])) {
				...
				if(isset($whois->Query['errstr'])) $winfo = implode($whois->Query['errstr'], "\n<br></br>");
				else
		...
		case 'proxy':
			if($allowproxy) exit(serialize($result));
...
function extract_block(&$plantilla, $mark, $retmark = '') {
	...
	if($retmark === false) $plantilla = substr($plantilla, 0, $start) . substr($plantilla, $final);
	else {
