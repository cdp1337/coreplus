...
class Spyc {
	...
	private function _yamlize($key, $value, $indent, $previous_key = -1, $first_key = 0, $source_array = null) {
		if(is_array($value)) {
			if(empty ($value)) return $this->_dumpNode($key, [], $indent, $previous_key, $first_key, $source_array);
	...
	private function _dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0, $source_array = null) {
		...
		if(is_string($value) &&
			((strpos($value, "\n") !== false || strpos($value, ": ") === 0 || strpos($value, "- ") === 0 ||
					strpos($value, "*") === 0 || strpos($value, "#") === 0 ||
					strpos($value, "<") === 0 || strpos($value, ">") === 0 ||
					strpos($value, '%') === 0 || strpos($value, '  ') === 0 ||
					strpos($value, "[") === 0 || strpos($value, "]") === 0 ||
					strpos($value, "{") === 0 || strpos($value, "}") === 0) ||
				strpos($value, "&") === 0 || strpos($value, "'") === 0 || strpos($value, "!") === 0 ||
				substr($value, -1, 1) == ':')
		) {
	...
	private function _doFolding($value, $indent) {
		...
		if($this->_dumpWordWrap !== 0 && is_string($value) && strlen($value) > $this->_dumpWordWrap) {
			...
			if($this->setting_dump_force_quotes && is_string($value) && $value !== self::REMPTY) $value =
				'"' . $value . '"';
	...
	private function _toType($value) {
		...
		if($is_quoted) {
			$value = str_replace('\n', "\n", $value);
			if($first_character == "'") return strtr(substr($value, 1, -1), ['\'\'' => '\'', '\\\'' => '\'']);
	...
	function revertLiteralPlaceHolder($lineArray, $literalBlock) {
		foreach($lineArray as $k => $_) {
			if(is_array($_)) $lineArray[ $k ] = $this->revertLiteralPlaceHolder($_, $literalBlock);
