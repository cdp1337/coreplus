...
EOD;
	...
	private function _setupComponent(){
		...
		$this->_iterator->scan($this->_base->getPath());
	...
	private function _setupCore(){
		...
		if(CDN_LOCAL_ASSETDIR)   $this->_iterator->addIgnore(CDN_LOCAL_ASSETDIR);
		if(CDN_LOCAL_PUBLICDIR)  $this->_iterator->addIgnore(CDN_LOCAL_PUBLICDIR);
		if(CDN_LOCAL_PRIVATEDIR) $this->_iterator->addIgnore(CDN_LOCAL_PRIVATEDIR);
		if(strpos(TMP_DIR_WEB, ROOT_PDIR) === 0) $this->_iterator->addIgnore(TMP_DIR_WEB);
		if(strpos(TMP_DIR_CLI, ROOT_PDIR) === 0) $this->_iterator->addIgnore(TMP_DIR_CLI);
